...
our %known_units = (
    ...
    # u  	-- atomic mass units alternative
    # dalton	-- 1 amu
...
sub evaluate_units {
    ...
    %output = %$fundamental_units
      if $@;    # this is what you get if there is an error.
    $output{'ERROR'} = $@ if $@;
    return %output;
}
...
# will then require a trailing parentheses match.  Otherwise, it just returns it all.
    ...
    $numerator = ""
      if ( $numerator eq '1' )
      ;    # prevent the 1 from getting recognized as anything.
...
# possibility that new unit is in denominator but not numerator, so check first. Assume zero power for missing fundamental.
            $unit_hash{$u} = ( $numerator_hash{$u} ? $numerator_hash{$u} : 0 ) -
              ( $denominator_hash{$u} ? $denominator_hash{$u} : 0 );
        }
    }
        ...
        foreach $f (@factors) {
            ...
            if ( $options->{hasChemicals} ) {
                if ( !defined &Chemical::Chemical::new ) {
                    die
"You need to load contextChemical.pl if you want to use chemicals as units.";
                }
                my $chemical = undef;
                $chemical = Chemical::Chemical->new($f);
                if ( defined $chemical && scalar @{ $chemical->{data} } > 0 ) {
                    my $power = 1
                      ; # reset power to 1 since it might have picked up a chemical charge as the power.
...
#piggyback the unit details (string,TeX) on the hash so that we can write them correctly on the screen
                    ...
                    $piggyback->{$f} = $chemical
                      ; #{string=> $chemical->{leading} . ' ' . $chemical->string, TeX=> $chemical->{leading} .'\,'. $chemical->TeX, base=>$unit_base};
...
# Adding a dash is fine, but if a user adds a custom unit with spaces, we want to honor it.
            ...
            @unitsNameArray = grep( /\s/x, @unitsNameArray );
            @unitsNamesArray2 =
              main::PGsort( sub { length( $_[0] ) > length( $_[1] ) },
                @unitsNameArray );
            my $unitsJoined = join '|', @unitsNamesArray2;
            my @splitUnits  = ( $f =~ m/($unitsJoined|\S+)/xg );
...
sub add_unit {
    ...
    unless ( ref($hash) eq 'HASH' ) {
        $hash = {
            'factor' =>
              1,    # this is 1 by default because it's a new "fundamental unit"
            "$unit" => 1
        };    # this is 1 by default because it's the base.
    }
...
# at this level since there's only individual units here.
    $string =~ s/[{}]//gx;
    my ( $unit_name, $power ) = split( /\^/x, $string );

    my @unitsNameArray = keys %$known_units;

    @unitNamesArray2 = main::PGsort( sub { length( $_[0] ) > length( $_[1] ) },
        @unitsNameArray );
    ...
    unless ( defined($unit_base) && defined( $known_units->{$unit_base} ) ) {
        ...
        #warn "TRYING UNIT PREFIX";
        ($unit_base) = $unit_name =~ /($unitsJoined)$/x;
        $unit_prefix = $unit_name =~ s/\s*($unitsJoined)\s*$//rx;
        $unit_prefix =~ s/\s//x;
...
# if not-strict mode, register this unit as a new unit with its own fundamentals
    ...
    # warn $known_units;
    # warn $unit_base;
    my %unit_name_hash = %{ $known_units->{$unit_base} }
      ;    # $reference_units contains all of the known units.
           #warn %unit_name_hash;
    my $u;
    ...
    foreach $u ( keys %unit_hash ) {
        if ( $u eq 'factor' ) {
            $unit_hash{$u} = ( $unit_name_hash{$u} * ( 10**$prefixExponent ) )
              **$power;    # calculate the correction factor for the unit
        }
        elsif ( $u eq 'parsed' ) {
            ...
            $unit_hash{$u} = $fundamental_unit *
              $power;  # calculate the power of the fundamental unit in the unit
        }
    }
    push @{ $unit_hash{'parsed'} },
      { prefix => $unit_prefix, base => $unit_base, power => $power };

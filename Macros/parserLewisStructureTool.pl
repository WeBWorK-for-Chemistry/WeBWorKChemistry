...
sub _parserLewisStructureTool_init {
	ADD_JS_FILE('https://cdn.jsdelivr.net/gh/limefrogyank/LewisStructuresWeb@latest/dist/bundle.js', 1, { defer => undef });
	main::PG_restricted_eval('sub LewisStructureTool { parser::LewisStructureTool->new(@_) }');
}

loadMacros('MathObjects.pl', 'contextChemical.pl', 'contextArbitraryString.pl');
...
sub ans_rule {
	...
	# ADD CODE HERE TO PRODUCE A BOX FOR DRAWING ON PAPER.
		...
		$kekuleOutput = $inputs->{$ans_name} if (defined $inputs->{$ans_name});
		my $showFormalCharges = $self->{cmpOptions}{showFormalCharges} ? "show-formal-charges" : "";
...
sub cmp_preprocess {
	...
	if ($main::displayMode ne 'TeX' && $main::displayMode ne 'PTX') {
		$ans->{student_ans} = '';#$main::PG->decode_base64($inputs->{"${ans_name}_svgOutput"});
		$ans->{preview_latex_string} = $main::PG->decode_base64($inputs->{"${ans_name}_svgOutput"});
	}
}
...
sub cmp {
	...
	if ($self->{cmpOptions}{checker}){
		$cmp->{rh_ans}{checker} = $self->{cmpOptions}{checker};
	} else {
		#$cmp->{rh_ans}{error_message} = "TTEST";
	...
	if ($main::displayMode ne 'TeX' && $main::displayMode ne 'PTX') {
		...
		if (! defined $kekuleJS){
			...
			my $bondOrderPenalty = defined $self->{cmpOptions}{bondOrderPenalty} ? $self->{cmpOptions}{bondOrderPenalty} : 0.25;
			my $lonePairPenalty = defined $self->{cmpOptions}{lonePairPenalty} ? $self->{cmpOptions}{lonePairPenalty} : 0.25;

			my $showFormalCharges = defined $self->{cmpOptions}{showFormalCharges} && $self->{cmpOptions}{showFormalCharges};
			my $formalChargePenalty = defined $self->{cmpOptions}{formalChargePenalty} ? $self->{cmpOptions}{formalChargePenalty} : 0.10;

			my $requirePerspective = defined $self->{cmpOptions}{requirePerspective} ? $self->{cmpOptions}{requirePerspective} : 1;
			my $perspectivePenalty = defined $self->{cmpOptions}{perspectivePenalty} ? $self->{cmpOptions}{perspectivePenalty} : 0.25;

			my $simpleKekule;
			eval { $simpleKekule = $json->decode($kekuleJS->{simpleKekule}); };

			my $correctMoleculeHash = Chemical::LewisStructure::generateHashFromLaTeXFormula($self->{data});
			...
			#  	warn %$bond;
			#  }
        	my $structureAnalysis = Chemical::LewisStructure::compareKekuleCTABAndHash($simpleKekule->{ctab}, $correctMoleculeHash);
			...
			if ($structureAnalysis->{atomsCorrect}){
				...
				$message .= "You're either missing atoms or some atoms aren't connected to the correct atoms. ";
				return $cmp;
			}
			if (!$structureAnalysis->{bondOrderCorrect}){
				my $bondOrderErrors = scalar @{$structureAnalysis->{badConnectors}};				
				$grade -= $bondOrderErrors * $bondOrderPenalty; # 25% off for each bond order error.
				$message .= "You have at least $bondOrderErrors <b>bond order</b> problem(s). ";
			}
			if (!$structureAnalysis->{lonePairsCorrect}){
				my $lonePairErrors = scalar @{$structureAnalysis->{badLonePairNodes}};
				$grade -= $lonePairErrors * $lonePairPenalty; # 25% off for each lone pair node error. (i.e. missing 3 lone pairs on one atom, marked off 25% once)
				$message .= "You have at least $lonePairErrors atom(s) with <b>lone pairs</b> incorrect. ";
			}
			if ($showFormalCharges && !$structureAnalysis->{formalChargesCorrect}){
				my $formalChargeErrors = scalar @{$structureAnalysis->{badFormalChargeNodes}};
				$grade -= $formalChargeErrors * $formalChargePenalty;  # default 10% off for each formal charge error. 
				$message .= "You have at least $formalChargeErrors atom(s) missing formal charges. ";
			}
			
			if ($requirePerspective && !$kekuleJS->{perspectiveCorrect}){
				$grade -= $perspectivePenalty;  # flat %25 off for leaving out dash-wedge bonds when tetrahedral, trigonal pyramidal, etc.
				$message .= "Your <b>perspective</b> is incorrect.  You may be missing dash/wedge bonds. ";
			}
			if ($grade < 0){
				$grade = 0;
			}
...
sub showCorrectAnswer {
	...
	my $answerHash = Chemical::LewisStructure::generateHashFromLaTeXFormula($formula, {saveSteps=>0, showFormalCharges=>$showFormalCharges});

	# With Lone Pairs adds lone pairs as an atom item (that gets hacked in drawing tool)
	my $answerMol = Chemical::LewisStructure::hashToMolFile($answerHash, {noCharges=>0, onlySingleBonds=>0, withLonePairs=>0});	
	#warn $answerMol;	
	my $correctAnswer = <<END_SCRIPT;
...
sub showSolutionGuide {
	...
	my $ans_name = $self->ANS_NAME;	
	my $iteration = $correctAnswerIteration++;
	my $showFormalCharges = defined $self->{cmpOptions}{showFormalCharges} && $self->{cmpOptions}{showFormalCharges} ? 1 : 0;
	my $showFormalChargesAttribute = $showFormalCharges ? "show-formal-charges" : "";
	#my $kekuleOutput = $inputs->{$ans_name} if (defined $inputs->{$ans_name});

	my $answerHash = Chemical::LewisStructure::generateHashFromLaTeXFormula($formula, {saveSteps=>1, showFormalCharges=>$showFormalCharges});
	...
	for my $step (@steps){
		...
		my $stepMol = Chemical::LewisStructure::hashToMolFile($step->{hash}, {noCharges=>0, onlySingleBonds=>0, withLonePairs=>1});	
		...
		if ($first) {
			$correctAnswer .= "<lewis-structure-canvas id='${ans_name}_answerTool_$iteration' mol='$stepMol' $showFormalChargesAttribute explicit-lone-pairs readonly></lewis-structure-canvas>\n";
		}
		$correctAnswer .= "</li>";
		$first=1;
		$iteration++;

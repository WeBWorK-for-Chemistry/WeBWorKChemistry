...
sub new {
	...
	unless (ref $num eq "ARRAY" || ref $num eq "InexactValue::InexactValue") {
...
sub TeXunits {
	...
	$units =~ s/\*/\\,/g; 	# replace multiplication with LaTeX small space
	$units =~ s/%/\\%/g; 	# replace quotient with LaTeX quotient
	$units =~ s/μ/\\mu /g; 	# replace unicode mu with LaTeX mu
	$units =~ s/ /\\,/g;  	# replace spaces with LateX small space example: adds space between 'fl oz'
	# warn $units;
